...
const Status UT_Print(string relation)
{
  ...
  if ((status = attrCat->getRelInfo(rd.relName, attrCnt, attrs)) != OK)
    return status;
  cerr << "[UT-print] AttrCnt " << attrCnt << endl;
  // compute width of output columns
  cerr << "[UT-print] Compute Width of output columns" << endl;
  int *attrWidth;
  if ((status = UT_computeWidth(attrCnt, attrs, attrWidth)) != OK)
    return status;
  cerr << "[UT-print] Compute Width of output columns finished" << endl;
  ...
  cerr <<  "Relation name: " << rd.relName << endl << endl;
